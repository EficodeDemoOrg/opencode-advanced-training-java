...
===
# Introduction
This section explains **context engineering**, the craft of exerting deliberate control over what the model sees before it answers, as opposed to prompt engineering, which relies on carefully typing a request (and hoping for the best). This is just a reference for the concepts. Hands-on practice lives in [hands-on/](../hands-on/).

## What "context" means in opencode
Everything the model sees on a given turn is its context: 
- your prompt,
- the files you attached with `@`, 
- the contents of `AGENTS.md`, 
- prior turns in the conversation, 
- and (if you addressed one) a subagent's own scoped context. 

Context engineering is choosing and shaping those inputs with purpose:
...
```text
@src/main/java/com/atlas/inventory/InventoryItem.java Write a Javadoc comment for the quote() method explaining what it escapes and why.
```
...
- you need the output to be in a specific, non-obvious format (e.g. a particular commit message style, a custom JSON error shape, a house code style),
- there are multiple reasonable ways to do the task and you want a specific one, or
...
A few-shot prompt doesn't have to hand-write its examples every time, though. Pointing at real, existing code with `@` and saying "follow this exact pattern" is few-shot prompting too:

```text
@src/main/java/com/atlas/inventory/InventoryItem.java The quote() method here is one example of this codebase's escaping style. Apply the same escaping style to the new exportToCsv() field-escaping helper you're about to write for InventoryRepository.
```

# Choosing between them
A useful default: start zero-shot. If the result's substance is correct but the *shape* is off, don't argue about it in prose, but show  examples of the required shape instead. 
|                           | Zero-shot                     | Few-shot                                                         |
|---------------------------|-------------------------------|------------------------------------------------------------------|
| Prompt length             | Short                         | Longer — costs more context                                      |
| Best for                  | Common, well-understood tasks | Specific formats, house conventions, cases where output is 'off' |
| Where the "examples" live | N/A                           | Inline in the prompt, or `@`-referenced existing code            |

This is usually faster than a paragraph of formatting instructions, and it's the same principle `AGENTS.md` uses at the project level: showing the model real conventions instead of describing them abstractly.
